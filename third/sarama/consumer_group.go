...
func (c *consumerGroup) Consume(ctx context.Context, topics []string, handler ConsumerGroupHandler) error {
	...
	fmt.Println("[Consume]CCCC")
	// Wait for session exit signal
	<-sess.ctx.Done()
	fmt.Println("[Consume]DDDD")
...
func (c *consumerGroup) loopCheckPartitionNumbers(topics []string, session *consumerGroupSession) {
	fmt.Println("[loopCheckPartitionNumbers]start")
	...
	fmt.Println("[loopCheckPartitionNumbers]AAAA")
	for {
		...
		fmt.Println("[loopCheckPartitionNumbers]BBBBB")
		select {
		case <-pause.C:
...
func (s *consumerGroupSession) consume(topic string, partition int32) {
	...
	if pom := s.offsets.findPOM(topic, partition); pom != nil {
		...
		fmt.Printf("[consume]AAAAoffset: %d \n", offset)
	}
	fmt.Printf("[consume]BBBBtopic:%s, partition: %d ,offset: %d \n", topic, partition, offset)
